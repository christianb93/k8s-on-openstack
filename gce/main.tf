...
variable "gcp_service_account_key" {
  type = string
  default = "../k8s-on-openstack-sa-key.json"
}
...
terraform {
  backend "local" {
    path = "../.state/tf/terraform.tfstate"
  }
}
...
output "inventory" {
  value = concat(
      ...
      [ {
        "groups"           : "['network_nodes','access_node']",
